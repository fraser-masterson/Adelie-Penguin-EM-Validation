...
# load the five model-ready datasets exported by the data prep script
tf_d1 = read_csv('output/model_datasets/tf1.csv')
tf_d2 = read_csv('output/model_datasets/tf2.csv')
i_d1 = read_csv('output/model_datasets/i1.csv')
i_d1.1 = read_csv('output/model_datasets/i1_1.csv')
i_d2 = read_csv('output/model_datasets/i2.csv')

# remove C61 as it is low quality throughout
tf_d1 = filter(tf_d1, grepl('C61', `Animal ID`) == 'FALSE')
...
tf_m1 = glmmTMB(as.factor(`Prey 1/0`) ~ `Window size (s)` + (1|`Animal ID`),
    family = binomial(link = 'logit'),
    data = tf_d1)
summary(tf_m1)

# build a prediction grid across window size to plot the fitted curve - data.frame() swaps
# spaces for dots in column names, so the colnames() line below renames it back (same fix
# reused throughout this script wherever a prediction grid is built)
df = data.frame('Window size (s)' = seq(0, 30, 0.01))
colnames(df)[colnames(df) == 'Window.size..s.'] = 'Window size (s)'
pred = predict(tf_m1, newdata = df, type = 'response', re.form = NA)
...
# pseudo R-squared for the model
explained_deviance = 100 * ((tf_m1$null.deviance - tf_m1$deviance) / tf_m1$null.deviance)
...
tf_m1log = glmmTMB(`Prey 1/0` ~ log(`Window size (s)`) + (1|`Animal ID`),
    family = binomial(link = 'logit'),
    data = tf_d1)
...
AIC(tf_m1, tf_m1log)
auc(roc(tf_d1$`Prey 1/0`, fitted(tf_m1log)))
...
# model shape vs data distribution
ggplot() +
...
# test additive vs interaction model structures
tf_m2_add = glmmTMB(`Prey 1/0` ~ scale(`SD window VeDBA`) + scale(`Mean window VeDBA`) + (1|`Animal ID`),
    family = binomial(link = 'logit'),
    data = tf_d1)
tf_m2_int = glmmTMB(`Prey 1/0` ~ scale(`SD window VeDBA`) * scale(`Mean window VeDBA`) + (1|`Animal ID`),
    family = binomial(link = 'logit'),
    data = tf_d1)

anova(tf_m2_add, tf_m2_int, test = 'Chisq')
AIC(tf_m2_add, tf_m2_int)
...
ggplot() +
    ...
    scale_fill_viridis_c(name = 'Probability', limits = c(0, 1), breaks = c(0, 0.25, 0.5, 0.75, 1)) +
    theme_minimal()
...
# full additive model with all four predictors and a dive-nested random effect
...
# generic function to test different model formula combinations for model selection
generate_models = function(response, predictors, random_effect) {
    ...
    # add the null model formula to the start of the list
    null_formula = as.formula(paste(response, '~ 1 + (1|', random_effect, ')'))
    formulas = c(list(null_formula), formulas)

    return(formulas)
}

# initiate predictor variables, response variables, and random effects to go through model selection function
...
# run model selection function to generate formulas
model_formulas = generate_models(response,
    predictors,
    random_effect)

# run a GLMM on each model formula in the list
model_list = lapply(model_formulas, function(formula) {
    glmmTMB(formula, data = tf_d1, family = 'binomial')
})

# create dataframe of all models and their performances
...
tf2_m1 = glm(`Prey 1/0` ~ `Time spent hunting (s)`,
    family = binomial(link = 'logit'),
    data = tf_d2)

AIC(tf2_m1)
100 * ((tf2_m1$null.deviance - tf2_m1$deviance) / tf2_m1$null.deviance)
...
tf2_m2 = glm(`Prey 1/0` ~ `Max dive depth`,
    family = binomial(link = 'logit'),
    data = tf_d2)

AIC(tf2_m2)
100 * ((tf2_m2$null.deviance - tf2_m2$deviance) / tf2_m2$null.deviance)
...
# 2.1: Prey presence absence ~ time spent hunting + max dive depth ----
...
AIC(tf2_m2.1)
100 * ((tf2_m2.1$null.deviance - tf2_m2.1$deviance) / tf2_m2.1$null.deviance)
...
tf2_m3 = glm(`Prey 1/0` ~ log(`Dive duration (s)`),
    family = binomial(link = 'logit'),
    data = tf_d2)

AIC(tf2_m3)
100 * ((tf2_m3$null.deviance - tf2_m3$deviance) / tf2_m3$null.deviance)
...
# same model selection approach as TRUE/FALSE dataset 1, using tf_d2 variables
...
# get all formula combinations from the variables
model_formulas_d2 = generate_models(response_d2,
    predictors_d2,
    random_effect_d2)

# run a GLMM on each formula combination
...
ggplot() +
    geom_density(data = i_d1.1, aes(x = `Window size (s)`))
...
# extract residuals and plot their autocorrelation function
...
# iterative polynomial degree test, comparing model fit by AIC
...
qqnorm(residuals(i_m3, type = 'response')); qqline(residuals(i_m3, type = 'response'), col = 'red')
...
qqnorm(residuals(i_m4, type = 'response')); qqline(residuals(i_m4, type = 'response'), col = 'red')
...
# subset to krill pce only, then flag whether each sighting was a swarm vs an individual
...
# filter to dives with at least one confirmed prey event before modelling total prey count
